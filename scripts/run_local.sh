...
# if [[ "$CONDA_DEFAULT_ENV" != "motus" ]]; then
#     echo "Error: Failed to activate motus environment"
#     exit 1
# fi
...
# PRETRAIN_CKPT=${5:-${PRETRAIN_CKPT:-"/share/home/bhz/test/latent_action_world_model/lawm/checkpoints/latent_action/stage2_1107/checkpoint_step_420000"}}
...
if [ "$NUM_GPUS" -eq 1 ]; then
    ...
    # Single GPU training
    TRAIN_CMD="python train/train.py --config $CONFIG_FILE --run_name $RUN_NAME --report_to tensorboard --pretrain_ckpt $PRETRAIN_CKPT"
    # Capture the actual exit code of the training command when piping to tee
...
# TRAIN_EXIT_CODE is set above to avoid pipeline exit code confusion
...
# RUN_NAME=${2:-${RUN_NAME:-"latent_action_pretrain_test"}}
GPU_IDS=${3:-${GPU_IDS:-"0,1,2,3,4,5,6,7"}}  # Default to GPU 0
# RESUME_FROM=${4:-${RESUME_FROM:-"/share/home/bhz/test/latent_action_world_model/lawm/checkpoints/robotwin/robotwin_pretrain_beat_block_hammer/checkpoint_step_10000"}}
CONFIG_FILE=${CONFIG_FILE:-"configs/lerobot.yaml"}
RUN_NAME=${RUN_NAME:-"lerobot_test"}
# PRETRAIN_CKPT=${5:-${PRETRAIN_CKPT:-"/share/home/bhz/test/latent_action_world_model/lawm/checkpoints/latent_action/stage2/checkpoint_step_420000"}}
...
# Log file for this run
LOG_FILE="logs_robotwin/local_${RUN_NAME}_gpus_${GPU_IDS//,/_}_$(date +%Y%m%d_%H%M%S).log"
...
if [ "$NUM_GPUS" -eq 1 ]; then
    ...
    if [ -n "$PRETRAIN_CKPT" ]; then
        TRAIN_CMD="$TRAIN_CMD --pretrain_ckpt $PRETRAIN_CKPT"
    fi
